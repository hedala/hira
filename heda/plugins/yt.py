...
def progress_hook(d):
    if d['status'] == 'downloading':
        ...
        bot.send_message(chat_id, f"Video indiriliyor. %{percentage}")
...
async def send_format_options(chat_id, url):
    ...
    await bot.send_message(chat_id, "İndirme formatını seçin:", reply_markup=InlineKeyboardMarkup(buttons))
...
async def search_youtube(chat_id, query):
    ...
    await bot.send_message(chat_id, "Arama sonuçları:", reply_markup=InlineKeyboardMarkup(buttons))
...
async def callback_query_handler(client, callback_query):
    ...
    if action == "video":
        ...
        await bot.send_document(callback_query.message.chat.id, file_path, caption=f"{info_dict['title']} - {format_id}")
        os.remove(file_path)
